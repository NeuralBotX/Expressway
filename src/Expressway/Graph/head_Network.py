...
def generate_nodes_and_edges(geopandas_data, pos_to_id):
    """
    :param geopandas_data: 一个 geopandas 类型的数据

    :param pos_to_id: 一个 对应着geopandas_data内经纬度信息的 位置与id映射表 {(经度,纬度):id}

    :return: 构建 network 的点的列表 及 这些点的列表关系

    % function -> 给定的 geopandas 类型的数据中提取道路网络的节点和边信息
    """
    edges = []
    nodes = []
    for index, row in geopandas_data.iterrows():
        ...
        for i in range(len(geo_row)):
            node = pos_to_id[(geo_row[i][0], geo_row[i][1])]
            front_node = pos_to_id[(geo_row[i - 1][0], geo_row[i - 1][1])]
            nodes.append(node)
            if i > 0:
